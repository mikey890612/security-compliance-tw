...
官方指引與現代系統之間存在三個缺口，本專案一併補上：

| 缺口 | 說明 |
|---|---|
| **語言** | 指引的程式範例只有 ASP.NET / Java / PHP |
| **層級** | 完全未涵蓋 API 層（物件級授權）與 AI 層（提示注入等） |
| **工具** | 描述「應如何實作」，但掃描器判定的是「污點分析追不追得出來」 |
...
| Skill | 時機 | 產出 |
|---|---|---|
| **`sec-audit`** | 送掃之前 / 拿到掃描報告之後 | `security-audit/findings.md`、`false-positives.md` |
| **`sec-harden`** | 寫程式的當下 | 專案的 `AGENTS.md`、Cursor `.mdc`、Cline / Windsurf / Copilot 規則檔 |
| **`sec-deliverables`** | 要交文件時 | 附表十查檢表、源碼查檢表、測試報告、威脅建模、RTM、委外 RFP |
...
5. **附表十查檢表本身有版本落差**——V3.2 內文新增 HTTP 安全標頭
   （4.5.3.4），但附件 1 查檢表未同步新增條目。詳見
   `references/controls-appendix10.md` 的「已知的版本落差」。

---

## 資料來源

本專案的知識庫係從下列公開指引**萃取並改寫**為 Go / Python / JavaScript
實作，並補上掃描器對應、API 層與 AI 層內容：

- 《Web 應用程式安全參考指引 V3.2》，數位發展部資通安全署，114.12.31
- 《安全軟體發展流程指引》《安全軟體設計參考指引》《安全軟體測試參考指引》，
  行政院資通安全辦公室，103 年

**原始 PDF 的著作權分屬上述機關，不隨本專案散布**，請自行至各機關網站取得。
