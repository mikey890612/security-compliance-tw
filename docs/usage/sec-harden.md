...
標記第一行的版本號就是產生時的 plugin 版本（`.mdc` 也在檔頭寫了一行）。
和 `plugin.json` 的版本比一下，就知道這個專案的規則檔是不是舊的；
重裝時它會回報「v0.1.0 → v0.2.0」這類變化，改了什麼見 `CHANGELOG.md`。
...
它讀的是濃縮速查（43 則），不是完整的 90 則——寫程式時不需要那麼詳細。
...
**Q：知識庫更新後怎麼同步到已安裝的專案？**
`git pull` 後重跑 `./install.sh` 更新 plugin，然後到各專案重跑一次安裝。標記區塊會被替換，
你自己寫的內容不受影響。
