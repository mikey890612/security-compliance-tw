...
請求解析產生的錯誤物件（multipart、JSON、表單解碼錯誤）**視為外部輸入**——
錯誤字串可能夾帶請求內容片段。記錄前一樣先過消毒函式。
...
真漏洞：身分鑑別失敗、授權被拒、重要資料新增修改刪除、管理者行為
這四類事件中，任一類在源碼中找不到對應的稽核寫入呼叫。

真漏洞：有紀錄但五要素缺一。最常見的兩種缺法是
「無法歸屬到特定使用者」與「時間戳記沒有時區或未經校時」。

真漏洞：只在成功路徑記錄，失敗路徑（`401` / `403` / 例外分支）沒有記錄。

通過：四類事件都由集中的稽核 API 寫入，
且能拿出單一筆實際輸出，逐項對上五要素。
...
真漏洞：日誌或稽核紀錄檔以 group 或 other 可寫的權限建立
（含 `0666`、`0777`、`0664`、`0620`）。

真漏洞：以截斷模式（`O_TRUNC`、`"w"`）開啟稽核紀錄檔，既有紀錄會被清空。

真漏洞：檔案路徑或權限位元由外部輸入決定（此時同時觸發路徑尋訪類的規則）。

通過：權限為 `0600` 或 `0640`、以附加模式開啟、路徑為程式或組態常數。

通過：應用程式不自行落地，日誌寫到標準輸出由平台收集，
且能提出平台端的權限與保存設定作為佐證。
