...
進入 HTTP 回應主體、回應標頭、或任何前端可見的頁面——前端的情況先看下方改寫即過與誤判，
兩者都不符合才是真漏洞。

真漏洞：生產環境設定啟用 debug、verbose error page 或框架預設錯誤頁。

改寫即過：自家前端程式把錯誤物件寫進頁面，但錯誤只來自瀏覽器 API，
或伺服器錯誤回應（含代理、LB 的錯誤頁）確定皆為常數——改顯示常數訊息就不再被標。

誤判：回應內容是程式內常數模板，僅串接系統產生的關聯 ID 或白名單欄位名，
且完整錯誤僅寫入日誌。

誤判：第三方套件把伺服器錯誤寫進頁面，而伺服器錯誤回應（含代理、LB 的錯誤頁）確定皆為常數——
套件檔不改，佐證寫明套件名稱、版本與常數回應的位置。
...
| gosec | G104（Audit errors not checked） | LOW | verified | testdata/scan-artifacts/open-source/20260905T084457Z/gosec.json#rule=G104（見 `references/scanner-verification-log.md`） |
...
- **日誌寫入失敗被忽略**——`logger.Write` 的回傳值幾乎沒人檢查。
  處置：若是稽核日誌（可歸責性用途），失敗必須有替代告警管道，屬真漏洞；
  若是除錯日誌，標記誤判並註明用途。
...
通過：錯誤被讀入條件判斷、被記錄、或被包裝往上傳，且失敗時控制流會中止。

通過：忽略的是無安全影響且無資料遺失風險的清理動作，並在原地註明理由
（用具體例外型別或 `contextlib.suppress`，不要用裸 except 或 `_ =`）。

改寫即過：同樣是無安全影響、無資料遺失風險的清理動作（例如隨後就刪除的暫存檔），
但寫成 `_ =` 或裸 except——實質安全，只是掃描器照標。改成上面的寫法或記錄錯誤即可。
...
error 分支，以及 Python／JS 的例外路徑（例外是常態控制流）。

真漏洞：Python／JS 的釋放動作寫在 `try` 區塊內，而非 `finally` / `with`。

改寫即過：Go 在每個 return 路徑都顯式釋放、沒用 `defer`，只有 panic 路徑沒釋放——
讀取端、寫入端皆同。改用 `defer`；寫入端要檢查 Close 錯誤時，用 `SAST-ERR-002` 的 defer 加具名回傳，
兩則同時滿足。有任何一條 return 或 error 路徑沒釋放，就回到第一條，是真漏洞。
...
通過：最外層統一 handler 捕捉廣義例外，但有完整記錄並回傳失敗狀態。

通過：捕捉具體型別後轉譯為領域錯誤並往上拋，原始錯誤以 `%w` 或 `from e` 保留。
