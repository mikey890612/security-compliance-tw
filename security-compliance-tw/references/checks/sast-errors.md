...
- **前端 JS 把錯誤物件寫進頁面**——Fortify 的 System Information Leak: External
  ...
  - 可能帶伺服器回應內容（`xhr.responseText`、`fetch` 讀出的錯誤本體）——真漏洞；
    伺服器的錯誤回應確定皆為常數時判誤判，根本修法在伺服器端（見過關寫法）
...
改寫即過：前端顯示的錯誤只來自瀏覽器 API，不含任何伺服器回應內容。
...
改寫即過：Go 為了檢查寫入端的 Close 錯誤，在每個 return 路徑都顯式 Close，
只有 panic 路徑沒關——改成 `SAST-ERR-002` 寫入端的 defer 加具名回傳，兩則同時滿足。
