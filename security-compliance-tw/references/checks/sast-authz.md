...
1. **授權寫成單一具名函式**（例如 `authz.Require`），全專案只有這一個入口。
   審查者才找得到唯一的決策點，佐證也只需指向它；自己執行掃描時，還能
   在 Fortify 註冊為 validation rule、在 Checkmarx 定義為 sanitizer、
   在 Semgrep 寫成 `pattern-not-inside`（選配）。散寫的 `if role == ...`
   任何工具與審查者都認不出來。
...
真漏洞：系統**完全沒有身分鑑別**，任何人都能呼叫非公開功能。這比「已認證但無權限」
更嚴重，同樣歸本則——連身分都沒有，就不可能有授權決策點。
此時只列本則一筆；**授權類**的 check（`SAST-AUTHZ-002`／`003`、`SAST-API-001`／`003`）併入處置，
補上身分鑑別後重跑模式 1 再逐則查。其他類別照常各自列出——例如 `SAST-LOG-003` 缺稽核事件
是獨立的發現，只是「使用者 ID」要等身分鑑別補上後才有。
...
- **程序啟動時需要 root（綁定低位埠、讀取憑證）**——
  ...
  註明降權位置。**若只是啟動需要 root、之後一直以 root 常駐，就是真漏洞。**

### 判定準則

真漏洞：檔案或目錄權限授予 owner 以外的寫入權（mode 含 group／other 的 `w`），
且該路徑存放設定檔、憑證、金鑰、可執行檔或應用程式資料。

真漏洞：服務常駐於 root（含容器未指定 `USER`），且無任何降權動作。
容器的隔離不算降權——逃逸後即為主機 root。

真漏洞：應用程式使用的資料庫帳號具備 DDL、SUPERUSER 或跨庫權限，
而其執行期功能僅需 DML。

真漏洞：雲端執行角色或存取策略使用萬用字元涵蓋動作或資源。
